...
def get_quantizer(
    ...
    quantizer_map = {
        ...
        "continuous_latent": qt.ContinuousLatentQuantizer,
    }
    klass = quantizer_map[quantizer]
    kwargs = dict_from_config(getattr(cfg, quantizer)) if hasattr(cfg, quantizer) and getattr(cfg, quantizer) is not None else {}
    if quantizer == "rvq":
        kwargs["dimension"] = dimension
    elif quantizer == "continuous_latent":
        kwargs["dimension"] = dimension
    return klass(**kwargs)
...
def get_encodec_autoencoder(encoder_name: str, cfg: omegaconf.DictConfig):
    ...
    elif encoder_name == "multihead_vae":
        from ..modules.multihead_vae_encoder import build_multihead_vae_encoder
        seanet_cfg = dict_from_config(getattr(cfg, "seanet"))
        encoder_override = seanet_cfg.pop("encoder", {})
        decoder_override = seanet_cfg.pop("decoder", {})
        sample_rate = getattr(cfg, "sample_rate", 32000)
        channels = getattr(cfg, "channels", 1)
        ratios = seanet_cfg.get("ratios", [8, 5, 4, 2])
        encoder = build_multihead_vae_encoder(
            sample_rate=sample_rate,
            channels=channels,
            ratios=ratios,
            n_filters=seanet_cfg.get("n_filters", 32),
            n_residual_layers=seanet_cfg.get("n_residual_layers", 1),
            causal=seanet_cfg.get("causal", False),
            **encoder_override,
        )
        decoder_dim = 512
        decoder_kwargs = {
            "channels": channels,
            "dimension": decoder_dim,
            "n_filters": seanet_cfg.get("n_filters", 32),
            "n_residual_layers": seanet_cfg.get("n_residual_layers", 1),
            "ratios": ratios,
            "activation": seanet_cfg.get("activation", "ELU"),
            "activation_params": seanet_cfg.get("activation_params", {"alpha": 1.0}),
            "norm": seanet_cfg.get("norm", "weight_norm"),
            "norm_params": seanet_cfg.get("norm_params", {}),
            "kernel_size": seanet_cfg.get("kernel_size", 7),
            "last_kernel_size": seanet_cfg.get("last_kernel_size", 7),
            "residual_kernel_size": seanet_cfg.get("residual_kernel_size", 3),
            "dilation_base": seanet_cfg.get("dilation_base", 2),
            "causal": seanet_cfg.get("causal", False),
            "pad_mode": seanet_cfg.get("pad_mode", "reflect"),
            "true_skip": seanet_cfg.get("true_skip", True),
            "compress": seanet_cfg.get("compress", 2),
            "lstm": seanet_cfg.get("lstm", 0),
            "disable_norm_outer_blocks": seanet_cfg.get("disable_norm_outer_blocks", 0),
        }
        decoder_kwargs.update(decoder_override)
        decoder = audiocraft.modules.SEANetDecoder(**decoder_kwargs)
        return encoder, decoder
    else:
